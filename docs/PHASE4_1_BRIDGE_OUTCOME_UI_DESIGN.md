...
**観察**: 沖縄 糖尿病の **粗 vs 年齢調整 の顕著な順位差**:
- 男: 粗16.3 (15位) → 調整20.8 (**2位**)、+27.6%
- 女: 粗10.5 (29位) → 調整9.7 (**2位**)、rate は微減だが順位が大きく上昇

これは Pattern 1 (Risk-Care 乖離) の解釈を補助する代表的 Outcome 例であり、Bridge Outcome 表示分離の必要性を示す。Pattern 1 自体は Risk・Demand/Use・Outcome の **複合構造** で判定するため、Outcome の逆転だけで Pattern 1 を断定するものではない。
...
**観察**: 山口 肺炎は粗 男1位/女2位 → 年齢調整後も男2位/女4位。Pattern 3 (供給+ × 結果悪) の解釈を補助する代表的 Outcome 例 — 「cap.homecare +104%」と並列に、補正後も肺炎・心疾患・腎不全が上位に観察される。Pattern 3 自体は Supply (cap proxy) と Outcome (年齢調整後でも高位) の構造関係で判定する。
