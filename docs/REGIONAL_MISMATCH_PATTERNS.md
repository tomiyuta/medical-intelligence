...
> MedIntel は「**地域医療の不一致パターンを発見する仮説生成装置**」である。  
>
> 単一指標で地域を評価せず、**複数軸 (リスク・受療・処方・供給・結果)** を独立に読み、**それらの不一致** を可視化することで、追加調査の動機付けを与える。  
>
> 本文書は、Phase 2E-1 (沖縄)、Phase 2E-2 (東北)、Phase 2E-3 (中四国・九州) の発見を **6つの不一致パターン** として統合する。
...
### Pattern 4: Supply-Outcome 整合 (Supply high / Outcome stable)
...
### Pattern 6: 都市低リスク・高機能集積 (Urban low-risk / high-function)
...
**示唆**: 大都市圏の若年・専門医療集積の構造。低リスクが医療提供の質によるものか、人口構造によるものかは分離不可能。  
**「リスク低 + 結果良好」は文脈依存**。  
...
**注**: 1県は複数パターンに該当することがある (例: 秋田は Pattern 2 と Pattern 5 の重複)。
...
> MedIntel は **6つの不一致パターン** を 47都道府県スケールで可視化できる:  
>
