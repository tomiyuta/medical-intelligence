...
| リスク | 影響 | 対策 |
|---|---|---|
| **NDB Risk side が現行と同型でない** | 圏域単位 P1 判定は県値 fallback または再設計版を要する | 県値 fallback + caveat 明示 / 別フェーズで再集計版を検討 |
...
| 4-3c-4 UI 改造 | 20-30 | 中 | **高** | 4-3c-3 完了後、または別フェーズで独立検討 |
| **合計 (完全実装)** | **60-90** | - | - | - |

---

→ Phase 4-3c-1 は **8 時間以内の audit** として独立実施可能。 → 完全実装 (60-90h) は他フェーズより大きく、reviewer 評価「後回し」と整合。
