...
| **真の制約** | NDB Risk side は二次医療圏別の表が一部存在するが、現行 MedIntel の県単位指標と同型ではなく、性年齢別再集計・分母再設計・秘匿値処理・標準化再設計が必要 (§12.5) |
...
| 領域 | 既存 | 二次医療圏単位の取得可能性 |
|---|---|---|
| **NDB 健診** (BMI/HbA1c/SBP/LDL 等 19 項目) | 県単位のみ | △ 第 10 回 NDB OpenData に「特定健診 性年齢・二次医療圏別 回答分布」、一部診療行為に「二次医療圏別算定回数」あり。ただし現行 MedIntel の県単位指標と同一粒度・同一形式ではなく、再集計・分母再設計・秘匿値処理が必要 (詳細 §12.5) |
| **NDB 受療率** (薬剤・診療) | 県単位のみ | △ 第 10 回 NDB OpenData の一部診療行為に「二次医療圏別算定回数」あり。ただし現行と同型ではなく再集計が必要 |
| **年齢調整死亡率** (人口動態統計) | 県単位 (2020) | △ e-Stat で市区町村別が部分公開、二次医療圏単位は集計必要 |
| **特定健診質問票** (14 項目) | 県単位 | △ NDB OpenData 分析サイト経由で「性年齢・二次医療圏別回答分布」を取得可能 (再集計が必要) |
| **介護給付実績** | 県単位 | △ 保険者単位 (市区町村) で公開、二次医療圏に集計可能 |

→ **核心制約**: Risk/Outcome side の NDB 由来データは **二次医療圏単位の表が一部存在するが、現行 MedIntel の指標 (county-level risk_rates / standardized rates) と同一粒度・同一形式ではない**。
→ 二次医療圏版を作るには、性年齢別再集計、分母設計 (受診者ベース vs 人口ベース)、秘匿値「－」のゼロ変換、年齢標準化の再設計が必要 (詳細 §12.5)。
→ 取得可能 (再集計込): 死亡率・介護給付実績、人口・病床・施設 (取得済)、NDB Risk side (一部、再設計必要)
...
リスク影響対策**NDB Risk side が現行と同型でない**圏域単位 P1 判定は県値 fallback または再設計版を要する県値 fallback + caveat 明示 / 別フェーズで再集計版を検討圏域単位サンプル小統計安定性低下95% CI + 圏域 n を必ず併記第 8 次医療計画の最新確認必要data 古い可能性Phase 4-3c-1 で必ず確認UI 複雑化誤読リスク増Phase 4-3c-4 は最後、guardrail 強化「圏域 = ランキング」誤読sample 小で順位は不安定percentile 表示禁止、distribution のみ圏域名表記揺れdata 整合性破壊Phase 4-3c-1 で master 整備
...
| NDB Risk side (健診・受療率) | △ **一部可能** | NDB OpenData 第10回 + NDB OpenData 分析サイト | 二次医療圏別の表は存在、ただし現行と同型ではなく再集計・標準化再設計が必要 (§12.5) |
...
### 12.5 NDB Risk side の制約 (reviewer 採択 修正版)

`docs/NDB_DATA_REQUEST.md` で記載済の Phase 1 取得仕様 (county-level) は維持。本フェーズの追加発見は以下:

#### 12.5.1 二次医療圏単位の表は一部存在する (reviewer 採択)

reviewer 確認 (2026-04-30):
- **第 10 回 NDB OpenData** に **「特定健診 性年齢・二次医療圏別 回答分布」** が存在 (質問票項目)
- 第 10 回 NDB OpenData の **一部診療行為に「二次医療圏別算定回数」** が存在
- **NDB OpenData 分析サイト** (https://ndbopendata-hub.com/) でも二次医療圏別 viewer 提供

→ 「NDB Risk side は二次医療圏単位 取得不可」は **不正確**。一部表は存在する。

#### 12.5.2 ただし現行 MedIntel と同型ではない

NDB OpenData の二次医療圏別表をそのまま MedIntel の `ndb_checkup_risk_rates.json` / `ndb_checkup_risk_rates_standardized.json` 形式に流し込むことは **できない**。理由:

| 項目 | 県単位 (現行) | 二次医療圏単位 (NDB) | 必要な再設計 |
|---|---|---|---|
| 集計粒度 | 県 × 検査値階層 | 性 × 年齢階級 × 二次医療圏 × 階層 | 性年齢別の再集計 |
| 分母 | 県内受診者全体 | 二次医療圏受診者 (一部秘匿) | 分母再設計 (受診者ベース vs 人口ベース) |
| 秘匿値 | なし | 「－」表記あり (件数 < 10) | 「－」→ 0 への明示変換ルール |
| 年齢標準化 | NDB 内標準人口で実施済 | 圏域単位で再計算が必要 | 性年齢別 raw → 直接法/間接法で再標準化 |
| 受診率 fallback | 県単位 | 圏域内人口を分母にした受診率の妥当性検証 | 圏域人口 source の整合 (社人研 vs e-Stat) |

#### 12.5.3 NDB OpenData 分析サイトの注意点

参照元で「－」(秘匿、件数 < 10) となっているものを「0」に変換して表示する仕様。研究用途で利用する際は、**ゼロと真の欠測 (data 不在) を区別する処理が必要**。

#### 12.5.4 結論 (NDB Risk side の Phase 4-3c での扱い)

- **△ 一部二次医療圏単位で取得可能**
- **ただし現行 MedIntel の指標形式と同型ではなく**、性年齢別再集計・分母再設計・秘匿値「－」のゼロ変換・年齢標準化の再設計が必要
- 4-3c-B-lite (Supply + 人口の最小最新化) の初期 scope からは外す
- NDB Risk side の二次医療圏化は別フェーズ (4-3c-Risk-NDB 等) で独立検討
...
| **D: NDB Risk side を別フェーズ化** | NDB Risk side の二次医療圏化は別フェーズ (4-3c-Risk-NDB) で独立検討、本フェーズでは Supply + 人口に focus | 0h (本フェーズ) |
...
### 12.8 ユーザーへの回答サマリ (reviewer 採択 修正版)

**「最新の二次医療圏データを収集することは可能か」に対する回答**:

**Yes, 部分的に可能**。Supply side と人口データは公式 source から取得可能 (JMAP + 社人研)。NDB Risk side も二次医療圏別の表は一部存在するが、現行 MedIntel と同型ではないため再設計が必要:
...
5. △ **NDB Risk side**: 第10回 NDB OpenData に「特定健診 性年齢・二次医療圏別 回答分布」+ 一部診療行為に「二次医療圏別算定回数」が **存在する**。ただし現行 MedIntel の県単位 risk_rates / standardized_rates と **同一粒度・同一形式ではない**。性年齢別再集計、分母再設計 (受診者 vs 人口)、秘匿値「－」のゼロ変換、年齢標準化の再設計が必要。

**最も効率的な path**: 案 B (JMAP + 社人研、16-20h) で Supply + 人口の最小最新化。NDB Risk side は別フェーズ (4-3c-Risk-NDB) で独立検討推奨。
