...
- **HEAD**: (next commit) — 在宅移行 補助分類 v0 47県 audit
...
**重要発見**:
1. **支援型10県は実存** — 5県sanity (5県+高齢化3県) で偶然不検出だっただけ。47県全体では判別力あり
2. **mixed 0件** — 3指標の高低が拮抗するケースは実データ上ほぼない (二値的に分かれる)
3. **慢性期シェア vs NDB在宅医療の相関 r = -0.090** (ほぼ無相関) — レビュー懸念「慢性期=療養依存=在宅薄」の単純構造は支持されない。v0で慢性期を支援型加点に含めても致命的誤読にはならない
...
**v0結論**: 5県sanityでは「支援型未検出」という偽の懸念があったが、47県全体では10:15 = 支援型:ギャップ型と判別力あり。v0ロジック維持でよい。

**Phase 2 (v1) 拡張候補** (audit 結果踏まえ更新):
1. ~~cap.homecare/cap.rehab 集計~~ — 5指標版へ拡張時に追加
2. 5指標版に拡張 (現在3指標) — peer review原仕様復元
3. z-score化 / percentile化 — gate閾値の統計的根拠
4. **gate を緩める検討** — 47県中22県(47%)が若年対象外で半数判定不可
5. **回復期/慢性期は床数per75plusへ変更** — シェアではなく絶対量に
6. **「療養依存型」の3分類目検討** — 慢性期 high × NDB在宅 low の組合せが意味を持つかは要検証 (現状 r=-0.09でほぼ独立)
