...
export default function HsaEmergencyPanel({ code, mob }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/hsa/emergency?code=${code}`).then(r => r.json()).then(x => { setD(x); setLoading(false); })
    ...
    <div style={{ background: '#fff', border: '1px solid #e8ecf0', borderRadius: 12, marginBottom: 20, overflow: 'hidden' }}>
      <button onClick={() => setOpen(o => !o)}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 700 }}>救急・職員体制</span>
      ...
      {open && (
        <div style={{ padding: '4px 18px 18px' }}>
          ...
          {!loading && !area && <div style={{ padding: 20, fontSize: 12.5, color: '#94a3b8' }}>この圏域の救急データは見つかりませんでした。</div>}
        </div>
      )}
    </div>
  );
