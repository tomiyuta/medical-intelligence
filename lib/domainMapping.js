...
// ════════════════════════════════════════════════════════════════════════════
// ことばチップ共通スケール (NdbView から移設 — 2026-07-09 手順1共有基盤)
// ════════════════════════════════════════════════════════════════════════════
// rose(高)/indigo(低)の中立発散色。赤=悪・緑=良の価値判断を輸入しない
// (受療率・処方量等の高低は受療行動・供給・疾病構造の複合であり良し悪しではない)。
// NdbView / DomainSupplyDemandBridge / 新部品(UnitDotLane・CheckupBinsHistogram・
// DeathWaffle100)のことばチップ単一ソース。Bridge→NdbView の循環importを避けるため
// UIコンポーネントではなく本libに置く。
...
// ════════════════════════════════════════════════════════════════════════════
// rank2: ドメインレンズ (疾患縦串フィルタ) — 逆引きユーティリティ
// ════════════════════════════════════════════════════════════════════════════
// NdbView 各レイヤーの行キー → 該当ドメインid[] を引くための逆引き辞書。
// risks(ndbQKey / riskKey / ndbHcMetric) / demand(patientSurveyKey) /
// utilization(codes) / supply(bedFuncKeys) / outcome(vitalCause,aamCause,additional)
// ※本対応は「薬効分類・ICD章の定義対応」であり因果連鎖の実証ではない (v0原則継承)。

// チップ表示順 (循環器→代謝→がん→脳血管→呼吸器→腎)
export const DOMAIN_ORDER = ['cardiovascular', 'diabetes_metabolic', 'cancer', 'cerebrovascular', 'respiratory', 'renal'];

// 死因ラベルの正規化 (括弧内注記を除去し vital_stats / age_adjusted 双方に突合)
const _normCause = (s) => (s || '').replace(/[（(].+?[)）]/g, '').trim();

export const DOMAIN_REVERSE = (() => {
  const push = (obj, key, id) => { if (key == null) return; (obj[key] = obj[key] || []).push(id); };
  const r = { ndbQKey: {}, riskKey: {}, hcMetric: {}, patientSurveyKey: {}, utilCode: {}, bedFuncKey: {}, vitalCause: {} };
  for (const id of DOMAIN_ORDER) {
    const d = DOMAIN_MAPPING[id];
    if (!d) continue;
    (d.risks || []).forEach((rk) => {
      push(r.ndbQKey, rk.ndbQKey, id);
      push(r.riskKey, rk.riskKey, id);
      push(r.hcMetric, rk.ndbHcMetric, id);
    });
    if (d.demand && d.demand.patientSurveyKey) push(r.patientSurveyKey, d.demand.patientSurveyKey, id);
    (d.utilization && d.utilization.codes ? d.utilization.codes : []).forEach((c) => push(r.utilCode, c, id));
    (d.supply && d.supply.bedFuncKeys ? d.supply.bedFuncKeys : []).forEach((k) => push(r.bedFuncKey, k, id));
    if (d.outcome && d.outcome.vitalCause) push(r.vitalCause, _normCause(d.outcome.vitalCause), id);
    if (d.outcome && d.outcome.aamCause) push(r.vitalCause, _normCause(d.outcome.aamCause), id);
    (d.outcome && d.outcome.additionalCauses ? d.outcome.additionalCauses : []).forEach((c) => push(r.vitalCause, _normCause(c.vitalCause), id));
  }
  // 重複除去
  for (const g of Object.values(r)) for (const k of Object.keys(g)) g[k] = [...new Set(g[k])];
  return r;
})();

// 特定ドメイン × レイヤー行キー の該当判定
// group: 'ndbQKey' | 'riskKey' | 'hcMetric' | 'patientSurveyKey' | 'utilCode' | 'bedFuncKey' | 'vitalCause'
export function rowInDomain(domainId, group, key) {
  if (!domainId || key == null) return false;
  const g = DOMAIN_REVERSE[group];
  if (!g) return false;
  const norm = group === 'vitalCause' ? _normCause(key) : key;
  return (g[norm] || []).includes(domainId);
}

// Layer4 (処方薬 疾患領域) は DRUG_DOMAIN の日本語ラベル軸で集計されるため、
// 薬効コード逆引きではなくドメインid→日本語ラベルの対応で突合する。
// cerebrovascular / renal は utilization=null (未整備) のため対応ラベル無し。
export const DOMAIN_TO_RX_LABEL = {
  cardiovascular: '循環器',
  diabetes_metabolic: '糖尿病・代謝',
  cancer: 'がん',
  respiratory: '呼吸器',
};

// ドメインの各断面(lens)の整備状況。null 断面は「この断面は未整備」表示に使う。
export function domainSectionStatus(domainId) {
  const d = DOMAIN_MAPPING[domainId];
  if (!d) return null;
  return {
    risks: d.risks ? d.risks.length : 0,
    demand: !!d.demand,
    utilization: !!d.utilization,
    supply: !!d.supply,
    outcome: !!d.outcome,
    demandNote: d.demandNote || null,
    utilizationNote: d.utilizationNote || null,
    supplyNote: d.supplyNote || null,
    isExperimental: !!d.isExperimental,
    experimentalNote: d.experimentalNote || null,
  };
}
